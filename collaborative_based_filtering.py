...
def collaborative_filtering_recommendations(data, target_user_id, top_n = 10):
  ...
  for user_index in similar_users_indices:
    ...
    if len(set(recommended_items)) > top_n * 3: # Get enough candidates then stop
        break
  
  recommended_items = list(set(recommended_items))
  recommended_items_details = data[data['ProdID'].isin(recommended_items)][['Name','ReviewCount','Brand','ImageURL','Rating']].drop_duplicates('Name')
